...
def sign_pdf(pdf_path, destination_sig_path):
    ...
    subprocess.call(["gpg", "--armor", "--output",
                    destination_sig_path, "--detach-sig", pdf_path])
...
def generate_verification_page(certificate, output_dir, grade=False):
    ...
    certificate_id = certificate[
        'graded_certificate_id'] if grade else certificate['certificate_id']

    generated_path = certificate[
        'graded_generated_path'] if grade else certificate['generated_path']
    download_url = certificate[
        'graded_download_url'] if grade else certificate['download_url']

    signature_filename = os.path.basename(generated_path) + ".sig"
    signature_generated_path = os.path.join(
        output_dir, certificate_id, signature_filename)
    ...
    new_valid_page = template_valid_page.format(NAME=certificate['name'],
                                                ...
                                                SIGNATURE=signature)
    writeStringToFile(new_valid_page, os.path.join(output_dir,
                      certificate_id, "valid.html"))
    ...
    writeStringToFile(new_verify_page, os.path.join(
        output_dir, certificate_id, "verify.html"))
...
def generate_certificates(student_name, certificate_id, filename, grade=None):
    ...
    pdfmetrics.registerFont(
        TTFont("Arial Unicode", fontpath('Arial Unicode.ttf')))

    pdfmetrics.registerFont(
        TTFont("Baskerville", fontpath('Baskerville.ttf')))
    pdfmetrics.registerFont(
        TTFont("Baskerville-Bold", fontpath('Baskerville-bold.ttf')))
    pdfmetrics.registerFont(
        TTFont("Baskerville-Italic", fontpath('Baskerville-Italic.ttf')))
    pdfmetrics.registerFont(TTFont(
        "Baskerville-BoldItalic", fontpath('Baskerville-BoldItalic.ttf')))
    ...
    addMapping(
        'Baskerville', 1, 1, 'Baskerville-BoldItalic')  # italic and bold
...
def generate_all_certificates(ungenerated_certificates, output_folder):
    ...
    for certificate in ungenerated_certificates:
        ...
        graded_output_name = os.path.join(
            output_folder, graded_output_id, filename)

        generate_certificates(name, certificate_id, filename=output_name)
        generate_certificates(name, graded_certificate_id,
                              filename=graded_output_name, grade=grade)

        generated_info = certificate.copy()
        generated_info.update(
            {'download_url': download_url(output_id, filename),
        ...
        if counter % 50 == 0:
            ...
            )), "Generated the certificates for", counter, "students"
...
def main():
    ...
    parser.add_argument('-u', '--uuids', type=argparse.FileType(
        'r'), default=None)  # No default=sys.stdin
    parser.add_argument('-i', '--input-file', type=argparse.FileType(
        'r'), default=None)  # No default=sys.stdin
    parser.add_argument(
    ...
    if arguments.input_file:
        ...
        input_data = [{'user_id': 1, 'name': u'Bridger Maxwell', 'grade': u'A', 'certificate_id': u'4f7c64dd3c284704b430a1a817b09a71', 'enabled': False, 'download_url': None, 'graded_download_url': None, 'graded_certificate_id': u'c792520fc0b949eebded6a791c0a51b4', 'id': 1}]
    ...
    if input_data:
        ...
        if not arguments.only_verify:
            input_data = generate_all_certificates(
                input_data, certificates_folder)
